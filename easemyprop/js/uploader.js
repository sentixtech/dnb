...
function fetchFiles(type = "", query = "", shareids = "", checkTypes = "") {
    ...
    $.post({
        ...
        success: function (response) {
            if (response.files != "") {
                displayFiles(response.files, checkType, shareId, shareid);
                selectFileCount();
            }
            currentPage = 1;
            setupPagination();
...
function displayFiles(fileData, types = "", selectedIds = "", selectInput) {
    ...
    $("#filesListShow").empty(); // Pehle existing files hatao

    files.forEach((file) => {
        if (uniqueFileNames.has(file.file_name)) {
            return; // Agar file_name already print ho chuka hai toh skip kar do
        }
        uniqueFileNames.add(file.file_name); // File name ko add karo Set me
        ...
        const isChecked = selectedIdArray.includes(file.id.toString()) ? "checked" : "";
        const fileActive = selectedIdArray.includes(file.id.toString()) ? "active" : "";

        $("#filesListShow").append(
            `<div class="col-lg-2 col-md-3 col-4 px-1">
              <label class="w-100 pointer" for="radio-${file.id}" >
                <input type="${type != "" ? type : "radio"}" class="d-none"
                  data-value="'${file.external_link}','${fileType}','${file.id}',${shareid},${file.original_name},${file.file_size}"
                  name="file-selection" value="${file.id}" ${isChecked} id="radio-${file.id}">
                <div class="file-item ${fileActive}" data-file-name="${file.file_name}">
                  ${previewContent}
                  <div class="text-start w-100 px-1 position-absolute bottom-0">
                    <div class="text-turncate">
                        <span class="text-dark font-bold">${file.original_name}</span>
                    </div>
                    <p class="m-0 p-0">${file.file_size}</p>
                    <button data-file-type="${fileType}" data-file-link="${file.external_link}"
                      data-file-name="${file.file_name}" class="btn btn-light d-flex align-items-center justify-content-center"
                      style="width:30px;height:30px;position: absolute; right: 10px; top: 1px; background-color: white; border-radius: 50%;"
                      data-bs-toggle="modal" data-bs-target="#previewImageUploader">
                        <i class="fas fa-eye text-primary"></i>
                    </button>
...
function debouncedFetchFiles() {
    ...
    debounceTimer = setTimeout(() => {
        let searchQuery = document.querySelector("input[name='KV-uploader-search']").value;
        let sortOrder = document.getElementById("filterSelect").value;
        let combinedQuery = `${searchQuery}::${sortOrder}`;
        fetchFiles(fetchType,combinedQuery, shareid,checkType);
    }, 500);
}
