...
$(document).ready(function () {
    ...
    $(".dynamic-btn").on("click", function () {
        ...
        const floorCategoriesValue = $("#floorCategoriesValue").val();

        // Check if the target exists and append content
        if (target.length) {
        ...
        if (floorCategoriesValue) {
            const categories = floorCategoriesValue.split(",");
            appendCategoryToDropdown(categories);
        }
    });
...
function appendCategoryToDropdown(categories) {
    const targetDropdown = $(".floorCategorySelect").last();
    categories.forEach(function (category) {
        category = category.trim();
        if (!targetDropdown.find(`option[value="${category}"]`).length) {
            targetDropdown.append(
                `<option value="${category}">${category}</option>`
            );
        }
    });
    targetDropdown.trigger("chosen:destroy").chosen();
}
